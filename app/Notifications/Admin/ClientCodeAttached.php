...
class ClientCodeAttached extends Notification implements ShouldQueue
{
    ...
    {
        $line = "Client code '{$this->clientCode->code}' was attached to user {$this->user->name}.";
        ...
        return (new MailMessage)
            ->subject('Client Code Attached (Admin)')
            ->line($line)
    ...
    public function toArray($notifiable): array
    {
        return [
            ...
            'message' => "Client code '{$this->clientCode->code}' was attached to user {$this->user->name}.",
        ];
    }
}
